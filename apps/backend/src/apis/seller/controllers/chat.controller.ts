...
export class SellerChatController {
  ...
  @ApiOperation({
    ...
    description: "특정 스토어의 모든 채팅방 목록을 조회합니다. 마지막 메시지 시간 기준으로 정렬됩니다. 자신이 소유한 스토어만 조회 가능합니다.",
  })
  ...
  async getChatRoomsByStore(
    ...
    return await this.chatService.getChatRoomsByStoreId(storeId, req.user.sub);
  }
  ...
  async sendMessage(
    ...
    return await this.chatService.sendMessage(roomId, dto.text, req.user.sub, "store");
  }
  ...
  async getMessages(
    ...
    return await this.chatService.getMessages(roomId, req.user.sub, "store", limitNum, cursor);
  }
}
