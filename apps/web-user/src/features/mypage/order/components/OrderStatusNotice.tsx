...
export function OrderStatusNotice({ order }: OrderStatusNoticeProps) {
  switch (order.orderStatus) {
    ...
    case OrderStatus.CANCEL_COMPLETED:
      ...
      return (
        <div className="mt-1 mb-4 border border-gray-100 rounded-lg overflow-hidden">
          <div className="flex items-center gap-1.5 px-2.5 py-2 bg-gray-50">
            <Icon name="warning" width={16} height={16} className="text-gray-400" />
            <p className="text-xs text-gray-700">판매자 요청으로 예약 취소되었습니다.</p>
          </div>
          <p className="py-2 px-2.5 text-xs text-gray-900">{order.sellerCancelReason}</p>
        </div>
      );
    ...
    case OrderStatus.NO_SHOW:
      return (
        <div className="mt-1 mb-4 border border-red-100 rounded-lg overflow-hidden">
          <div className="flex items-center gap-1.5 px-2.5 py-2 bg-red-50">
            <Icon name="warning" width={16} height={16} className="text-red-400" />
            <p className="text-xs text-gray-700">노쇼 처리된 예약입니다.</p>
          </div>
          <p className="py-2 px-2.5 text-xs text-gray-900">
