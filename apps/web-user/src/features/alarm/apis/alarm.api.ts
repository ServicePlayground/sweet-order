...
export const alarmApi = {
  // 주문 알림 목록 (`/v1/user/notifications`)
  getNotifications: async (params?: {
    page?: number;
    limit?: number;
    unreadOnly?: boolean;
  }): Promise<AlarmNotificationListResult> => {
    const response = await userClient.get("/notifications", { params });
  ...
  getUnreadCount: async (): Promise<number> => {
    const response = await userClient.get("/notifications/unread-count");
    return (response.data.data as { count: number }).count;
  },

  // 단일 읽음
  markRead: async (notificationId: string): Promise<void> => {
    await userClient.patch(`/notifications/${notificationId}/read`);
  },

  // 전체 읽음
  markAllRead: async (): Promise<void> => {
    await userClient.patch("/notifications/read-all");
  },
};
