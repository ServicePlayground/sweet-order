...
sequenceDiagram
    ...
    Note over S,U: http://localhost:3001/auth/login?returnUrl=http%3A//localhost%3A3002/dashboard
...
- **통합 인증 페이지**: 사용자페이지, 판매자페이지 모두 인증이 필요할 때 web-user의 `/auth/login` 경로로 리다이렉트
