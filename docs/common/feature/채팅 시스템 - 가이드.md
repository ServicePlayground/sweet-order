...
- 연결 옵션:
  ...
  - `transports: ["websocket", "polling"]` - WebSocket 우선 시도, 실패 시 polling으로 폴백
...
현재는 **WebSocket과 polling을 모두 지원**합니다:

- 프론트엔드: `transports: ["websocket", "polling"]`
- 백엔드: `transports: ["websocket", "polling"]`

**동작 방식:**

- Socket.IO는 WebSocket 연결을 우선 시도합니다
- WebSocket 연결이 실패하거나 지원되지 않는 환경에서는 자동으로 polling으로 폴백합니다
- WebSocket은 양방향 통신이 가능하여 실시간성이 더 우수합니다
- HTTP polling보다 서버 리소스 사용량이 적습니다

#### 인프라 고려사항

**AWS App Runner 환경**

현재 AWS App Runner를 사용 중이며, WebSocket 연결이 가능한 환경에서는 WebSocket을 사용하고, 그렇지 않은 경우 polling으로 자동 폴백됩니다.

**향후 ECS 마이그레이션 시 장점**

AWS ECS(Elastic Container Service)로 마이그레이션할 경우:

1. **ALB(Application Load Balancer)와 함께 사용 시 WebSocket 완벽 지원**
...
- **현재**: WebSocket + polling 사용 (WebSocket 우선, 실패 시 polling 폴백)
- **동작**: 환경에 따라 자동으로 최적의 전송 방식을 선택
