...
- 전송한 메시지도 서버에서 브로드캐스트되므로, `onNewMessage` 리스너를 통해 자동으로 수신되어 UI에 표시됩니다.
...
WebSocket 연결 (Socket.IO)
    ├─ JWT 토큰 인증 (우선순위: auth.token → query.token → Authorization 헤더)
    ├─ 토큰 검증 및 사용자 정보 조회
    └─ 연결 성공 (client.data에 userId, userType 저장)
    ↓
프론트엔드 → ChatSocketService.joinRoom(roomId)
    ↓
WebSocket → "join-room" 이벤트 전송 { roomId }
    ↓
...
- 연결 옵션:
  ...
  - `transports: ["polling"]` - HTTP polling만 사용 (현재 App Runner 환경의 안정성을 위해)
    - **향후 개선**: ECS로 마이그레이션 후 `transports: ["websocket", "polling"]`로 변경 예정
...
- 인증: JWT 토큰 전달 (우선순위: `auth.token` → `query.token` → `Authorization` 헤더)
  - Socket.IO v4 권장 방식인 `auth.token`을 우선 사용
  - 하위 호환성을 위해 `query.token`도 지원
  - 표준 HTTP 방식인 `Authorization` 헤더도 지원
- 주요 메서드:
...
- Socket.IO 서버 사용
- 기본 Namespace 사용 (명시적 namespace 설정 없음)
- Path: `/socket.io/` (배포 환경에서 WebSocket 연결 안정성을 위해 명시)
- 연결 옵션:
  - `transports: ["polling"]` - HTTP polling만 사용 (현재 App Runner 환경의 안정성을 위해)
    - **향후 개선**: ECS로 마이그레이션 후 `transports: ["websocket", "polling"]`로 변경 예정
...
- 연결 시 JWT 토큰 검증 (우선순위: `auth.token` → `query.token` → `Authorization` 헤더)
...
1. **이벤트 리스너 유지**
   - Socket.IO가 자동으로 이벤트 리스너를 유지합니다
   - 재연결 후에도 등록된 리스너가 계속 작동합니다
   - `onNewMessage()`로 등록한 리스너는 재연결 시에도 자동으로 유지됩니다
...
- **NO_TOKEN**: 토큰이 제공되지 않은 경우
  - 클라이언트에 `error` 이벤트로 전달 후 연결 종료
...
- 연결 실패 시 사용자에게 알림 표시
- 재연결 시도 중 연결이 끊어진 경우 사용자에게 경고 알림 표시
- 정상적인 클라이언트 종료(`io client disconnect`)인 경우 알림 표시하지 않음

### WebSocket 전송 방식 개선 계획

#### 현재 상태

현재는 배포 환경의 안정성을 위해 **HTTP polling만 사용**하고 있습니다:

- 프론트엔드: `transports: ["polling"]`
- 백엔드: `transports: ["polling"]`

#### 향후 개선 방향

**WebSocket 전송 방식 추가 필요**

성능과 효율성을 위해 추후 `transports: ["websocket", "polling"]`로 변경해야 합니다:

- WebSocket을 우선 시도하고, 실패 시 polling으로 폴백
- WebSocket은 양방향 통신이 가능하여 실시간성이 더 우수
- HTTP polling보다 서버 리소스 사용량이 적음

#### 인프라 요구사항

**AWS App Runner의 제한사항**

현재 AWS App Runner를 사용 중이지만, WebSocket을 안정적으로 지원하려면 **AWS ECS(Elastic Container Service)**로 마이그레이션이 필요합니다:

1. **App Runner의 WebSocket 제한**
   - App Runner는 WebSocket 연결을 완전히 지원하지 않을 수 있음
   - 로드 밸런서 레벨에서 WebSocket 업그레이드가 제대로 처리되지 않을 수 있음
   - 장기 연결(long-lived connections) 관리에 제약이 있을 수 있음

2. **ECS의 장점**
   - ALB(Application Load Balancer)와 함께 사용 시 WebSocket을 완벽하게 지원
   - WebSocket 연결 업그레이드(HTTP → WebSocket)를 정상적으로 처리
   - 장기 연결 관리에 최적화
   - 더 세밀한 리소스 제어 및 스케일링 가능

3. **마이그레이션 시 고려사항**
   - ECS 클러스터 설정
   - ALB 설정 (WebSocket 지원 활성화)
   - 서비스 디스커버리 설정
   - 헬스 체크 설정
   - 로그 및 모니터링 설정
...
- **현재**: HTTP polling만 사용 (App Runner 환경)
- **향후**: WebSocket + polling 사용 (ECS 환경으로 마이그레이션 필요)

WebSocket을 추가하려면 App Runner에서 ECS로 인프라를 변경해야 합니다.
