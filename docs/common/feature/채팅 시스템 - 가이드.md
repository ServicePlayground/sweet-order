...
스위트오더의 채팅 시스템은 사용자와 판매자 간의 실시간 메시지 교환을 지원합니다. REST API와 WebSocket(Socket.IO)을 결합하여 구현되었으며, 하이브리드 아키텍처를 통해 안정성과 실시간성을 모두 확보했습니다.
...
사용자 → [메시지 입력 후 전송 버튼 클릭]
    ↓
프론트엔드 → POST /user/chat-room/[roomId]/messages { text }
    ↓
백엔드 → ChatService.sendMessage()
    ↓
...
- 메시지는 REST API로만 전송합니다. 클라이언트에서 직접 WebSocket으로 메시지를 보내지 않습니다.
...
1. **메시지 영속성 보장**
   - 모든 메시지는 REST API를 통해 데이터베이스에 저장
   - WebSocket 연결이 끊겨도 메시지 손실 없음
...
3. **안정성**
   - WebSocket 연결 실패 시에도 REST API로 메시지 전송 가능
   - 재연결 후 이전 메시지 조회 가능

4. **일관성**
   - 메시지 전송은 항상 REST API로 처리
   - 클라이언트 로직 단순화
