...
본 문서는 SweetOrder 프로젝트의 백엔드 코드와 문서를 분석하여, 실제 운영 단계에서 발생할 수 있는 문제점들을 정리한 가이드입니다.
...
**위치**: `apps/backend/src/infra/database/prisma.service.ts` 38-56번 라인
...
- App Runner가 비정상 인스턴스에 트래픽을 계속 라우팅
...
### 2. 마이그레이션 실패 시 프로세스 강제 종료

**위치**: `apps/backend/src/scripts/migration.ts` 33-36번 라인
...
- 마이그레이션 실패 시 `process.exit(1)`로 강제 종료
- App Runner가 컨테이너를 재시작하지만 마이그레이션 문제가 해결되지 않으면 무한 재시작 루프 발생 가능
- 마이그레이션 실패 원인에 대한 상세 로그 부족

**위험성**:

- 잘못된 마이그레이션으로 인한 서비스 완전 중단
- 마이그레이션 롤백 전략 부재
- 운영 환경에서 데이터베이스 스키마 변경 시 위험

**해결 방안**:

1. 마이그레이션 전 스키마 백업
2. 마이그레이션 실패 시 자동 롤백 메커니즘
3. 마이그레이션 실패 시 알림 발송 (SNS, Slack 등)
4. 마이그레이션 실행 전 검증 단계 추가

---

### 3. 환경 변수 검증 부족

**위치**: `apps/backend/src/common/utils/loadSecretsFromEnv.ts` 3-22번 라인
...
### 5. 외부 API 호출 시 타임아웃 및 재시도 설정 부족

**위치**: `apps/backend/src/modules/business/services/nts-api.service.ts` 30-35번 라인
...
### 6. Health Check 엔드포인트 검증 부족

**위치**: `apps/backend/src/main.ts` 51-53번 라인
...
- App Runner가 비정상 인스턴스를 정상으로 인식하여 트래픽 라우팅
- 실제 장애 발생 시 감지 불가
...
### 7. Rate Limiting 설정이 전역으로만 적용

**위치**: `apps/backend/src/app.module.ts` 33-37번 라인
...
**위치**: Prisma 트랜잭션 사용 부분 (예: `cart.service.ts`, `product-like.service.ts`)
...
### 10. 외부 API 에러 처리 일관성 부족

**위치**: `apps/backend/src/modules/business/services/nts-api.service.ts` 88-99번 라인
...
### 11. 파일 업로드 크기 제한이 하드코딩

**위치**: `apps/backend/src/modules/upload/constants/upload.constants.ts` 67번 라인
